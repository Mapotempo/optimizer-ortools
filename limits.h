...
class NoImprovementLimit : public SearchLimit {
  ...
  virtual bool Check() {
    if (!first_solution_
        && ((nbr_solutions_with_no_better_obj_ > solution_nbr_tolerance_ && solution_nbr_tolerance_ > 0) || (1e-6 * (absl::GetCurrentTimeNanos() - start_time_) > time_out_ && initial_time_out_ > 0))) {
      limit_reached_ = true;
    } else if (first_solution_ && init_duration_ > 0 && 1e-6 * (absl::GetCurrentTimeNanos() - start_time_) > init_duration_) {
      limit_reached_ = true;
    }
  ...
  virtual bool AtSolution() {
    ...
    if (minimize_ && objective->Min() * 1.01 < best_result_) {
      ...
      nbr_solutions_with_no_better_obj_ = 0;
      if (initial_time_out_ > 0) time_out_ = std::max(initial_time_out_, time_out_coef_ * 1e-6 * (absl::GetCurrentTimeNanos() - start_time_));
    } else if (!minimize_) {
...
class LoggerMonitor : public SearchLimit {
  public:
    LoggerMonitor(const TSPTWDataDT &data, RoutingModel * routing, RoutingIndexManager * manager, int64 min_start, int64 size_matrix, bool debug, bool intermediate, ortools_result::Result* result, std::string filename, const bool minimize = true)
      ...
      if (intermediate_) {
        ...
        for (int route_nbr = 0; route_nbr < routing_->vehicles(); route_nbr++) {
          ...
          for (int64 index = routing_->Start(route_nbr); !routing_->IsEnd(index); index = routing_->NextVar(index)->Value()) {
            ...
            activity->set_start_time(routing_->GetMutableDimension(kTime)->CumulVar(index)->Min());
            activity->set_current_distance(routing_->GetMutableDimension(kDistance)->CumulVar(index)->Min());
            ...
            for (std::size_t q = 0 ; q < data_.Quantities(RoutingIndexManager::NodeIndex(0)).size(); ++q) {
          ...
          RoutingIndexManager::NodeIndex nodeIndex = manager_->IndexToNode(routing_->End(route_nbr));
          end_activity->set_index(data_.ProblemIndex(nodeIndex));
          end_activity->set_start_time(routing_->GetMutableDimension(kTime)->CumulVar(routing_->End(route_nbr))->Min());
          end_activity->set_current_distance(routing_->GetMutableDimension(kDistance)->CumulVar(routing_->End(route_nbr))->Min());
  ...
  virtual SearchLimit* MakeClone() const {
    // we don't to copy the variables
    return solver_->RevAlloc(new LoggerMonitor(data_, routing_, manager_, min_start_, size_matrix_, debug_, intermediate_, result_, filename_, minimize_));
  }
  ...
  std::vector<double> GetFinalScore() {
    return {(best_result_ / 1000000.0), 1e-9 * (absl::GetCurrentTimeNanos() - start_time_), (double)iteration_counter_};
  }
...
LoggerMonitor * MakeLoggerMonitor(const TSPTWDataDT &data, RoutingModel * routing, RoutingIndexManager * manager, int64 min_start, int64 size_matrix, bool debug, bool intermediate, ortools_result::Result* result, std::string filename, const bool minimize = true) {
  return routing->solver()->RevAlloc(new LoggerMonitor(data, routing, manager, min_start, size_matrix, debug, intermediate, result, filename, minimize));
}
}  //  namespace operations_research
